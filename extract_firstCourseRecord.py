...
def extract_firstCourseRecord(file):
    ...
    try:
        if wushi:
            ...
            if chiefandhistory_text[0]:
                ...
                # 拉取病历特点
                chief = regex.search(r'(?<=病例特点|病历特点).*?(?=病史介绍)', chiefandhistory_text[0])
                # print(chief)
                if chief:
                    text.append(chief[0])
                else:
                ...
                # 拉取病史介绍
                chief = regex.search(r'(?<=病史介绍).*?(?=中医望闻、切|中医望、闻、切)', chiefandhistory_text[0])
                # print(chief)
                if chief:
                    text.append(chief[0])
                else:
                ...
                item = regex.findall(r'(?<=中医望闻、切、诊).*?(?=体格检查)', chiefandhistory_text[0])
                if item:
                    text.append(item[0])
                else:
                    text.append('*')
                ...
                # 拉取体格检查
                item = regex.findall(r'(?<=体格检查).*?(?=辅助检查)', chiefandhistory_text[0])
                if item:
                    text.append(item[0])
                else:
                    text.append('*')
                ...
                # 拉取初步诊断
                item = regex.search(r'(?<=初步诊断).*?(?=中医辨病辨证依据)', chiefandhistory_text[0])
                # print(chief)
                if item:
                    text.append(item[0])
                else:
                ...
                # 拉取中医辨病辨证依据
                item = regex.search(r'(?<=中医辨病辨证依据).*?(?=西医诊断依据)', chiefandhistory_text[0])
                # print(chief)
                if item:
                    text.append(item[0])
                else:
                ...
                #拉取西医诊断依据
                item = regex.search(r'(?<=西医诊断依据).*?(?=鉴别诊断)', chiefandhistory_text[0])
                # print(chief)
                if item:
                    text.append(item[0])
                else:
                ...
                #拉取鉴别诊断
                item = regex.search(r'(?<=鉴别诊断).*?(?=诊疗计划)', chiefandhistory_text[0])
                # print(chief)
                if item:
                    text.append(item[0])
                else:
                ...
                #拉取诊疗计划
                item = regex.search(r'(?<=诊疗计划).*?(?=(\d{4}-\d{1,2}-\d{1,2}))', chiefandhistory_text[0])
                # print(chief)
                if item:
                    text.append(item[0])
                else:
